...
def copyAndLog(fromFile, toFile, logfile):
    try:
        ...
        logfile.write("")
        logfile.write("-----ERROR-----\n")
        logfile.write("Copy Environmental Error on " +
                      fromFile + " to " + toFile + "\n")
        logfile.write(str(e))
        logfile.write("")
        ...
        logfile.write("-----ERROR-----\n")
        logfile.write(
            "Copy  Non Enviromental Error on " +
            fromFile +
            " to " +
            toFile +
            "\n")
...
def chmodAndLog(file, permission, logfile):
    try:
        ...
        logfile.write("")
        logfile.write("-----ERROR-----\n")
        logfile.write("CHMOD  Non Enviromental Error on " +
                      file + " to " + str(permission) + "\n")
        logfile.write(str(e))
        logfile.write("---------------\n")
    else:
        logfile.write(
...
def chownAndLog(file, uid, gid, logfile):
    try:
        ...
        logfile.write("-----ERROR-----\n")
        logfile.write(
            "CHOWN Non Enviromental Error on " +
            file +
            " to " +
            str(uid) +
            "\n")
...
def tap():
    ...
    ts = time.time()
    dateTimeVal = datetime.datetime.fromtimestamp(
    ...
    if avrFlag == 0:
        ...
        subprocess.Popen(
            'sudo -u pi /home/pi/.octoprint/scripts/cancelPrint.sh',
            shell=True,
        ...
        subprocess.Popen(
            'sudo -u pi /home/pi/oprint/bin/octoprint client post_json \'/api/plugin/mgsetup\' \'{"command":"mgLog","stringToLog":"' +
            logText +
            '","priority":"0"}\'',
            shell=True,
            stdout=subprocess.PIPE)
...
def shutdownHold():
    ...
    logText = dateTimeVal + \
        "System was Shutdown because the RAMBo reset button  / reset line was held for more than 10 seconds at "
    persistentLogFileName = "/home/pi/.octoprint/logs/cancel_and_shutdown_from_reset.log"
    persistentLogfile = open(persistentLogFileName, "a")
    persistentLogfile.write(logText + "\n")
    persistentLogfile.close()
...
def resetPasswordHold():
    ...
    subprocess.Popen(
        'sudo -u pi /home/pi/.octoprint/scripts/stopOctoprint.sh',
        shell=True,
        stdout=subprocess.PIPE)
    ...
    copyAndLog(
        ...
        "/etc/netconnectd.yaml.userold",
        logfile)
    copyAndLog(
        "/home/pi/.octoprint/config.yaml",
        "/home/pi/.octoprint/config.yaml.userold",
        logfile)
    copyAndLog(
        "/home/pi/.octoprint/users.yaml",
        "/home/pi/.octoprint/users.yaml.userold",
        logfile)
    copyAndLog(
        "/etc/network/interfaces",
        "/etc/network/interfaces.userold",
        logfile)

    # copy and log canonical backups to working files
    copyAndLog("/home/pi/.octoprint/config.yaml.backup",
               "/home/pi/.octoprint/config.yaml", logfile)
    copyAndLog("/home/pi/.octoprint/users.yaml.backup",
               "/home/pi/.octoprint/users.yaml", logfile)
    copyAndLog("/home/pi/.octoprint/scripts/interfaces",
               "/etc/network/interfaces", logfile)
    copyAndLog(
        "/etc/netconnectd.yaml.backup",
        "/etc/netconnectd.yaml",
        logfile)
    copyAndLog(
        "/home/pi/.octoprint/scripts/config.txt.original",
        "/boot/config.txt",
        logfile)
    ...
    logText = dateTimeVal + "System was restored because the RAMBo reset button  / reset line was held for more than 60 seconds at See " + \
        logFileName + "for more information"
    persistentLogFileName = "/home/pi/.octoprint/logs/cancel_and_shutdown_from_reset.log"
    persistentLogfile = open(persistentLogFileName, "a")
    persistentLogfile.write(logText + "\n")
    persistentLogfile.close()
    ...
    if buttonState != prevButtonState:
        ...
        if (t - prevTime) >= rebootHoldTime:  # Button held more than 'rebootHoldTime'?
            ...
            if buttonState:       # Button released?
                if tapEnable:       # Ignore if prior shutdownHold()
