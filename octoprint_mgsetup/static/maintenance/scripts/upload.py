...
# has long moves going and tries to update from one major version of
# marlin to another.
...
subprocess.check_output(
    'echo $(date) > /home/pi/.octoprint/logs/firmware.log',
    shell=True)

print("Canceling prints and disconnecting ")
subprocess.check_output(
    'echo Canceling prints and disconnecting >> /home/pi/.octoprint/logs/firmware.log',
    shell=True)

print("Starting Update and Upload Process")
subprocess.check_output(
    'echo Starting Update and Upload Process >> /home/pi/.octoprint/logs/firmware.log',
    shell=True)
...
#                                   | $$
#                                   |__/
# clean and compile first - this will hopefully give some extra time
# incase of some sort of long move or other issue going on with the
# printer
os.chdir("/home/pi/m3firmware/")

try:
    subprocess.check_output(
        'platformio run --target clean >> /home/pi/.octoprint/logs/firmware.log',
        shell=True)
...
try:
    subprocess.check_output(
        'platformio run >> /home/pi/.octoprint/logs/firmware.log',
        shell=True)
...
subprocess.check_output(
    'echo Storing  previous firmware settings >> /home/pi/.octoprint/logs/firmware.log',
    shell=True)
sys.stdout.flush()
...
ser = serial.Serial(
    ...
    xonxoff=False,
    rtscts=False,
    dsrdtr=False)  # start serial port to upload script
...
while lineCount < 20:  # max out to 100 lines
    ...
    if data_raw.startswith(
            "ok"):  # ok means we're done  - should put something
        # print "done"
        lineCount = 20
...
if responseLineProbe is not None:
    matcher = re.match (
        r'FIRMWARE_NAME:Marlin ([0-9]\.[0-9]\.[0-9]\.?[0-9]?\.?[0-9]?) \(Github\) SOURCE_CODE_URL:https://github.com/MakerGear/m3firmware PROTOCOL_VERSION:1.0 MACHINE_TYPE:(.*) EXTRUDER_COUNT:([1-2])',
        responseLine)
    zprobeMatcher = re.match (r'Cap:Z_PROBE:([0-1])', responseLineProbe)

if (matcher is None) or (zprobeMatcher is None):
    print("Warning: Could not detect previous marlin firmware and settings.")
    subprocess.check_output(
        'echo Warning: Could not detect previous marlin firmware and settings. >> /home/pi/.octoprint/logs/firmware.log',
        shell=True)
...
if matcher is not None:
    while lineCount < 100:  # max out to 100 lines
        ...
        if data_raw.startswith("ok"):
            ...
            matcher = re.match (
                r'echo:  M206 X(-?[0-9]{1,3}\.[0-9][0-9]) Y(-?[0-9]{1,3}\.[0-9][0-9]) Z(-?[0-9]{1,3}\.[0-9][0-9])',
                responseLine)
            if matcher:
            ...
            matcher = re.match (
                r'echo:  M218 T1 X(-?[0-9]{1,3}\.[0-9][0-9]) Y(-?[0-9]{1,3}\.[0-9][0-9]) Z(-?[0-9]{1,3}\.[0-9][0-9])',
                responseLine)
            if matcher:
            ...
            matcher = re.match (
                r'echo:  M851 Z(-?[0-9]\.[0-9][0-9])',
                responseLine)
            if matcher:
                m851Z = matcher.group(1)


if m206X is not None and m206Y is not None and m206Z is not None:
    ...
    subprocess.check_output(
        'echo Could not read previous M503 M206 - Please proceed to Quick Check after firmware has uploaded >> /home/pi/.octoprint/logs/firmware.log',
        shell=True)

    sys.stdout.flush()


if extruderCount is None:
    print("Could not read number of extruders - Please proceed to Quick Check after firmware has uploaded")
    subprocess.check_output(
        'echo Could not read number of extruders - Please proceed to Quick Check after firmware has uploaded >> /home/pi/.octoprint/logs/firmware.log',
        shell=True)

elif int(extruderCount) == 2:

    if m218X is not None and m218Y is not None and m218Z is not None:
        ...
        subprocess.check_output(
            'echo Could not read previous M503 M218 - Please proceed to Quick Check after firmware has uploaded >> /home/pi/.octoprint/logs/firmware.log',
            shell=True)
...
if zprobe is None:
    ...
    if m851Z is not None:
        ...
        print("Could not read previous M503 M851 - Please proceed to Quick Check after firmware has uploaded")
        subprocess.check_output(
            'echo Could not read previous M503 M851 - Please proceed to Quick Check after firmware has uploaded >> /home/pi/.octoprint/logs/firmware.log',
            shell=True)
...
subprocess.check_output(
    'echo Starting Upload >> /home/pi/.octoprint/logs/firmware.log',
    shell=True)
sys.stdout.flush()
...
try:
    subprocess.check_output(
        '/home/pi/.platformio/packages/tool-avrdude/avrdude -cwiring -p atmega2560 -P/dev/ttyS0 -b115200 -D -Uflash:w:.pioenvs/megaatmega2560/firmware.hex >> /home/pi/.octoprint/logs/firmware.log',
        shell=True)
    print("Upload Successful ")
    subprocess.check_output(
        'echo Upload Sucessful >> /home/pi/.octoprint/logs/firmware.log',
        shell=True)
    sys.stdout.flush()
except subprocess.CalledProcessError:
    print("Failed to Upload, quitting")
    subprocess.check_output(
        'echo Failed to Upload, quitting >> /home/pi/.octoprint/logs/firmware.log',
        shell=True)
    sys.stdout.flush()
    quit()
...
if matcher is not None:
    ...
    subprocess.check_output(
        'echo Restoring parameters >> /home/pi/.octoprint/logs/firmware.log',
        shell=True)
    sys.stdout.flush()
    ...
    if m206String is not None:
        ...
        subprocess.check_output(
            ...
            " Z" +
            m206Z +
            " >> /home/pi/.octoprint/logs/firmware.log",
            shell=True)

        # print "write M206"
    if m851String is not None:
